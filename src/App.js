...
const App = () => {
  ...
  const [activeCharId, setActiveCharId] = useState('egg'); // 横滑り防止: 現在固定されているキャラ
  const [activeStage, setActiveStage] = useState(1);       // 横滑り防止: 現在到達しているステージ
  ...
  const currentStageCalculated = useMemo(() => {
    ...
    if (s < 600) return 1; // Phase 1
    if (s < 3600) return 2; // Phase 2
    if (s < 18000) return 3; // Phase 3
    if (s >= 36000) return 10; // 最終形態
    const ratio = s / 36000;
    return 4 + Math.floor(ratio * 5.9); // Stage 4 ~ 9
  }, [totalSeconds]);

  useEffect(() => {
    if (!user || !isDataLoaded) return;
    
    // 進化の瞬間（または初期データが無くて計算上のステージが高い場合）のみキャラを変更し固定する
    if (currentStageCalculated > activeStage) {
      ...
      if (nextDisc.length !== discoveredIds.length) {
        setDiscoveredIds(nextDisc);
      }
  ...
  // --- Fun Loading Text for Image Gen ---
  useEffect(() => {
  ...
  // Image Management (Fail-Fast & Fixed Load)
  useEffect(() => {
    let isMounted = true;
  ...
  // --- API Handlers (Content Generation & Prompt Logic) ---
  ...
  const generateContent = async () => {
    ...
    if (lengthType === 'single') {
      lengthInstruction = "Exactly ONE single sentence.";
    } else if (lengthType === 'short') {
      lengthInstruction = "A short paragraph consisting of exactly 3 to 5 sentences.";
    } else if (lengthType === 'essay') {
      lengthInstruction = "A detailed multi-paragraph text consisting of at least 3 paragraphs and approximately 150-250 words.";
    }

    let genreInstruction = "";
    if (genre === 'business') {
      genreInstruction = "Topic: Professional business scenario, office communication, or corporate strategy. Vary the industry wildly.";
    } else if (genre === 'daily') {
      genreInstruction = "Topic: Everyday casual conversation, daily routines, or practical life situations. Vary the setting wildly.";
    } else if (genre === 'academic') {
      genreInstruction = "Topic: Academic subject, scientific discovery, historical event, or university-level discussion. Vary the field wildly.";
    } else if (genre === 'creative') {
      if (lengthType === 'single' || lengthType === 'short') {
        genreInstruction = "Target: A very famous but RANDOMLY SELECTED quote from a movie, a wise saying by a historical figure, or an iconic line from a popular manga/anime. The 'context' field MUST be the work title or person name.";
      } else {
        genreInstruction = "Target: A highly unique and detailed review, analysis, or storytelling about a famous movie, historical figure, or iconic anime. The 'context' field MUST be the title or subject.";
      }
    }
    ...
    } catch (e) { setErrorMsg("生成に失敗しました。時間をおいてもう一度お試しください。"); } finally { setIsLoading(false); }
  };

  // --- Pronunciation Scoring Handler ---
